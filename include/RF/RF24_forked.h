/*
 Forked from https://github.com/maniacbug/RF24
 Need to have low level access to RF24 chip
...
class RF24 {
   private:
    ...
    uint8_t spi_txbuff[32 + 1];  // SPI transmit buffer (payload max 32 bytes +
                                 // 1 byte for the command)
#endif
    bool p_variant;                /* False for RF24L01 and true for RF24L01P */
    uint8_t payload_size;          /**< Fixed size of payloads */
    bool dynamic_payloads_enabled; /**< Whether dynamic payloads are enabled. */
    uint8_t pipe0_reading_address[5]; /**< Last address set on pipe 0 for
                                         reading. */
   ...
   public:
     ...
     * instance and send in the unique pins that this chip is connected to.
     *
     ...
     * 2. Do not call write() while in this mode, without first calling
     * stopListening().
     ...
     * radio.write(&data,sizeof(data));
     * @endcode
     * @return True if the payload was delivered successfully and an ACK was
     * received, or upon successfull transmission if auto-ack is disabled.
     */
    bool write(const void* buf, uint8_t len);

    /**
     * New: Open a pipe for writing via byte array. Old addressing format
  retained
     * for compatibility.
     *
  ...
  s   *
     ...
     *
     * @note After calling startListening(), a basic radio will consume
     * about 13.5mA at max PA level. During active transmission, the radio will
     * consume about 11.5mA, but this will be reduced to 26uA (.026mA) between
     * sending. In full powerDown mode, the radio will consume approximately
     * 900nA (.0009mA)
     *
     * @code
     * radio.powerDown();
    ...
    /**
     * Write for single NOACK writes. Optionally disables
     * acknowledgements/autoretries for a single write.
     *
     ...
     *FIFO to clear by issuing txStandBy() or ensure appropriate time between
     *transmissions.
     *
     * @code
     * Example (Partial blocking):
     *
     *			radio.writeFast(&buf,32);  // Writes 1 payload to the
     *buffers txStandBy();     		   // Returns 0 if failed. 1 if success.
     *Blocks only until MAX_RT timeout or success. Data flushed on fail.
     *
     *			radio.writeFast(&buf,32);  // Writes 1 payload to the
     *buffers
     *			txStandBy(1000);		   // Using extended timeouts, returns 1 if
     *success. Retries failed payloads for 1 seconds before returning 0.
     * @endcode
     *
     * @see txStandBy()
     * @see write()
    ...
    /**
     * WriteFast for single NOACK writes. Disables acknowledgements/autoretries
     * for a single write.
     *
     * @note enableDynamicAck() must be called to enable this feature
     * @see enableDynamicAck()
     ...
     *
     *			radio.writeBlocking(&buf,32,1000); //Wait up to 1 second to write 1
     *payload to the buffers
     *			txStandBy(1000);     			   //Wait up to 1 second for the payload
     *to send. Return 1 if ok, 0 if failed.
     ...
     * @return True if the payload was loaded into the buffer successfully false
     *if not
     */
     ...
     * operating mode.
     *
     * @note When transmitting data in rapid succession, it is still recommended
     *by the manufacturer to drop the radio out of TX or STANDBY-II mode if
     *there is time enough between sends for the FIFOs to empty. This is not
     *required if auto-ack is enabled.
     *
     ...
     *			bool ok = txStandBy();     //Returns 0 if failed. 1 if
     *success.
     *					  				   //Blocks only until MAX_RT timeout or success. Data
     *flushed on fail.
     * @endcode
    ...
    /**
     * This function allows extended blocking and auto-retries per a user
     *defined timeout
     * @code
     ...
     *					  				    //Blocks only until user defined timeout or
     ...
     * @warning Only three of these can be pending at any time as there are only
     * 3 FIFO buffers.<br> Dynamic payloads must be enabled.
     * @note Ack payloads are handled automatically by the radio chip when a
     * payload is received. Users should generally write an ack payload as soon
     * as startListening() is called, so one is available when a regular payload
     * is received.
     * @note Ack payloads are dynamic payloads. This only works on pipes 0&1 by
     * default. Call enableDynamicPayloads() to enable on all pipes.
     *
     ...
     *
     * @note Optimization: This function now leaves the CE pin high, so the
     * radio will remain in TX or STANDBY-II Mode until a txStandBy() command is
     * issued. Can be used as an alternative to startWrite() if writing multiple
     * payloads at once.
     ...
     * rule. Allow the FIFO to clear by issuing txStandBy() or ensure
    ...
    /**
     * This function is mainly used internally to take advantage of the auto
     * payload re-use functionality of the chip, but can be beneficial to users
     * as well.
     *
     * The function will instruct the radio to re-use the data in the FIFO
     * buffers, and instructs the radio to re-send once the timeout limit has
     * been reached. Used by writeFast and writeBlocking to initiate retries
     * when a TX failure occurs. Retries are automatically initiated except with
     * the standard write(). This way, data is not flushed from the buffer until
     * switching between modes.
     *
     * @note This is to be used AFTER auto-retry fails if wanting to resend
     * using the built-in payload reuse features.
     * After issuing reUseTX(), it will keep reending the same payload forever
     * or until a payload is written to the FIFO, or a flush_tx command is
     * given.
     */
    void reUseTX();
     ...
     * operation. May be required in specific cases after stopListening() , if
     ...
     *default the library will cease operation when a failure is detected. This
     *should allow advanced users to detect and resolve intermittent hardware
     *issues.
     *
     * In most cases, the radio must be re-enabled via radio.begin(); and the
     *appropriate settings applied after a failure occurs, if wanting to
     *re-enable the device immediately.
     *
     * The three main failure modes of the radio include:
     *
     * Writing to radio: Radio unresponsive - Fixed internally by adding a
     *timeout to the internal write functions in RF24 (failure handling)
     *
     * Reading from radio: Available returns true always - Fixed by adding a
     *timeout to available functions by the user. This is implemented internally
     *in  RF24Network.
     *
     * Radio configuration settings are lost - Fixed by monitoring a value that
     *is different from the default, and re-configuring the radio if this
     ...
     *    radio.begin();                       // Attempt to re-configure the
     *radio with defaults radio.failureDetected = 0;           // Reset the
     *detection value radio.openWritingPipe(addresses[1]); // Re-configure pipe
     *addresses radio.openReadingPipe(1,addresses[0]); report_failure(); //
     *Blink leds, send a message, etc. to indicate failure
     *  }
     * @endcode
     */
     ...
     * requested radio.write(&data,32,0);  // Sends a payload using
     * auto-retry/autoACK
     * @endcode
     */
     ...
     * Get the CRC length
     * <br>CRC checking cannot be disabled if auto-ack is enabled
     * @return RF24_CRC_DISABLED if disabled or RF24_CRC_8 for 8-bit or
     ...
     *users to mask those interrupts to prevent them from generating a signal on
     *the interrupt pin. Interrupts are enabled on the radio chip by default.
     *
     * @code
     ...
     *
     * When responding to payloads, faster devices like ARM(RPi) are much faster
     * than Arduino:
     * 1. Arduino sends data to RPi, switches to RX mode
     * 2. The RPi receives the data, switches to TX mode and sends before the
     * Arduino radio is in RX mode
     * 3. If AutoACK is disabled, this can be set as low as 0. If AA/ESB
     * enabled, set to 100uS minimum on RPi
     *
     * @warning If set to 0, ensure 130uS delay after stopListening() and before
     * any sends
     */
    ...
    /**
     *
     * On all devices but Linux and ATTiny, a small delay is added to the CSN
     * toggling function
     *
     * This is intended to minimise the speed of SPI polling due to radio
     * commands
     *
    ...
    // Making all private functions public to be able to use low level stuff
    // private:
     ...
     * means we're less likely to effectively leverage our FIFOs and pay a
     * higher AVR runtime cost as toll.
     *
     ...
     * standby.  Please see data sheet for a much more detailed description of
     * this pin.
     */
    void ce(bool level);
     ...
     * 'features' are enabled.  See the datasheet for details.
     */
 ...
 *
 * This is an example of how to use the RF24 class to communicate on a basic
 * level. Configure and write this sketch to two different nodes. Put one of the
 * nodes into 'transmit' mode by connecting with the serial monitor and <br>
 * sending a 'T'. The ping node sends the current time to the pong node, which
 * responds by sending the value back. The ping node can then see how long the
 * whole cycle took. <br>
 * @note For a more efficient call-response scenario see the
 * GettingStarted_CallResponse.ino example.
 * @note When switching between sketches, the radio may need to be powered down
 * to clear settings that are not "un-set" otherwise
 */
 ...
 *
 * This example continues to make use of all the normal functionality of the
 * radios including the auto-ack and auto-retry features, but allows
 * ack-payloads to be written optionlly as well. <br> This allows very fast
 * call-response communication, with the responding radio never having to switch
 * out of Primary Receiver mode to send back a payload, but having the option to
 * switch to <br> primary transmitter if wanting to initiate communication
 * instead of respond to a commmunication.
 */
 ...
 *
 * This example demonstrates how to send multiple variables in a single payload
 * and work with data. As usual, it is generally important to include an
 * incrementing value like millis() in the payloads to prevent errors.
 */
 ...
 * <br> with the serial monitor and sending a 'T'.  The data transfer will
 * begin, with the receiver displaying the payload count. (32Byte Payloads) <br>
 */
 ...
 * auto-retries/auto-reUse to increase reliability in noisy or low signal
 * scenarios. <br>
 *
 * Write this sketch to two different nodes.  Put one of the nodes into
 * 'transmit' mode by connecting with the serial monitor and sending a 'T'.  The
 * data <br> transfer will begin, with the receiver displaying the payload count
 ...
 * call-response communication, with the responding radio never having to switch
 * out of Primary Receiver mode to send back a payload, but having the option to
 * if wanting<br> to initiate communication instead of respond to a
 * commmunication.
 */
 ...
 * <b>Update: TMRh20</b><br>
 * This is an example of how to user interrupts to interact with the radio, and
 * a demonstration of how to use them to sleep when receiving, and not miss any
 * payloads.<br> The pingpair_sleepy example expands on sleep functionality with
 * a timed sleep option for the transmitter. Sleep functionality is built
 * directly into my fork of the RF24Network library<br>
 */

/**
 * @example pingpair_irq_simple.ino
 * <b>Dec 2014 - TMRh20</b><br>
 * This is an example of how to user interrupts to interact with the radio, with
 ...
 * efficient system.  It is just like the GettingStarted_CallResponse example,
 * but the<br> ping node powers down the radio and sleeps the MCU after every
 ...
 * This is an example of how to use the RF24 class to communicate with ATtiny85
 * and other node. <br>
 */
 ...
 *
 * This is an example of how to use payloads of a varying (dynamic) size on
 * Arduino.
 */
 ...
 *
 * This is a python example for RPi of how to use payloads of a varying
 * (dynamic) size.
 */
 ...
 * @li Consumed with a public interface that's similar to other Arduino standard
libraries
 *
 * @section News News
 *
 ...
 * - <a href="https://github.com/stewarthou/Particle-RF24">Particle
Photon/Core</a> fork available
 ...
 * - New layout for <a href="Portability.html">easier portability:</a> Break out
defines & includes for individual platforms to RF24/utility
 ...
 *
 *
 * If issues are discovered with the documentation, please report them <a
href="https://github.com/TMRh20/tmrh20.github.io/issues"> here</a>
 *
 * <br>
 * @section Useful Useful References
 *
 *
 * @li <a href="http://tmrh20.github.io/RF24/classRF24.html"><b>RF24</b> Class
Documentation</a>
 * @li <a
href="https://github.com/TMRh20/RF24/archive/master.zip"><b>Download</b></a>
 * @li <a href="https://github.com/tmrh20/RF24/"><b>Source Code</b></a>
 * @li <a
href="http://tmrh20.blogspot.com/2014/03/high-speed-data-transfers-and-wireless.html"><b>My
Blog:</b> RF24 Optimization Overview</a>
 * @li <a
href="http://tmrh20.blogspot.com/2016/08/raspberry-pilinux-with-nrf24l01.html"><b>My
Blog:</b> RPi/Linux w/RF24Gateway</a>
 * @li <a
 ...
 *
 * @li <a href="http://tmrh20.github.io/RF24Network"> <b>RF24Network:</b> OSI
Network Layer for multi-device communication. Create a home sensor network.</a>
 * @li <a href="http://tmrh20.github.io/RF24Mesh"> <b>RF24Mesh:</b> Dynamic Mesh
Layer for RF24Network</a>
 * @li <a href="http://tmrh20.github.io/RF24Ethernet"> <b>RF24Ethernet:</b>
TCP/IP Radio Mesh Networking (shares Arduino Ethernet API)</a>
 * @li <a href="http://tmrh20.github.io/RF24Audio"> <b>RF24Audio:</b> Realtime
Wireless Audio streaming</a>
 * @li <a href="http://tmrh20.github.io/">All TMRh20 Documentation Main Page</a>
 *
 * **More Information and RF24 Based Projects**
 *
 * @li <a href="http://TMRh20.blogspot.com"> Project Blog: TMRh20.blogspot.com
</a>
 * @li <a href="http://maniacalbits.blogspot.ca/"> Maniacal Bits Blog</a>
 * @li <a href="http://www.mysensors.org/">MySensors.org (User friendly sensor
networks/IoT)</a>
 * @li <a href="https://github.com/mannkind/RF24Node_MsgProto">
RF24Node_MsgProto (MQTT)</a>
 * @li <a href="https://bitbucket.org/pjhardy/rf24sensornet/"> RF24SensorNet
</a>
 * @li <a href="http://www.homeautomationforgeeks.com/rf24software.shtml">Home
Automation for Geeks</a>
 * @li <a href="https://maniacbug.wordpress.com/2012/03/30/rf24network/">
Original Maniacbug RF24Network Blog Post</a>
 * @li <a href="https://github.com/maniacbug/RF24"> ManiacBug on GitHub
(Original Library Author)</a>
 *
 *
 * <br>
 *
 ...
 *
 * @li <a href="Arduino.html"><b>Arduino</b></a> (Uno, Nano, Mega, Due, Galileo,
etc)
 * @li <a href="ATTiny.html"><b>ATTiny</b></a>
 * @li <a href="Linux.html"><b>Linux devices</b></a>( <a
href="RPi.html"><b>RPi</b></a> , <a href="Linux.html"><b>Linux SPI userspace
device</b></a>, <a href="MRAA.html"><b>MRAA</b></a> supported boards ( Galileo,
Edison, etc), <a href="LittleWire.html"><b>LittleWire</b></a>)
 * @li <a href="CrossCompile.html"><b>Cross-compilation</b></a> for linux
devices
 * @li <a href="Python.html"><b>Python</b></a> wrapper available for Linux
devices
 *
 * <br>
 * **General µC Pin layout** (See the individual board support pages for more
info)
 *
...
LittleWire [2]          |    RPI     | RPi -P1 Connector |
 *
|-----|----------|-------------|--------------------|-----------------|-------------------------|------------|-------------------|
 * |  1  |   GND    |   GND       |     pin 4          |    pin 14       | GND
| rpi-gnd    |     (25)          |
 * |  2  |   VCC    |   3.3V      |     pin 8          |    pin  1       |
regulator 3.3V required | rpi-3v3    |     (17)          |
 * |  3  |   CE     |   digIO 7   |     pin 2          |    pin 12       | pin
to 3.3V             | rpi-gpio22 |     (15)          |
 ...
 * These pins must be chosen and designated by the user, in RF24
radio(ce_pin,cs_pin); and can use any
 * available pins.
 *
 * <br>
 * @section ARD_DUE Arduino Due
 *
 * RF24 makes use of the extended SPI functionality available on the Arduino
Due, and requires one of the
 * defined hardware SS/CS pins to be designated in RF24
radio(ce_pin,cs_pin);<br>
 ...
 *
 * RF24 supports alternate SPI methods, in case the standard hardware SPI pins
 ...
 *
 * The Serial Port (UART) on Arduino can also function in SPI mode, and can
double-buffer data, while the
 ...
 *
 *
 * @note SPI_UART on Mega boards requires soldering to an unused pin on the
chip. <br>See
 * https://github.com/TMRh20/RF24/issues/24 for more information on SPI_UART.
 *
 * @page ATTiny ATTiny
 *
 * ATTiny support is built into the library, so users are not required to
include SPI.h in their sketches<br>
 * See the included rf24ping85 example for pin info and usage
 *
 * Some versions of Arduino IDE may require a patch to allow use of the full
program space on ATTiny<br>
 * See
https://github.com/TCWORLD/ATTinyCore/tree/master/PCREL%20Patch%20for%20GCC for
ATTiny patch
 *
...
free to use for application <br>
 *    Circuit idea from
...
<br> *	For best settletime delay value in RF24::csn() the timingSearch3pin.ino
sketch can be used.         <br>
 *    This configuration is enabled when CE_PIN and CSN_PIN are equal, e.g. both
3                      <br>
 *    Because CE is always high the power consumption is higher than for 5 pins
solution                <br>
 * @code
 * ^^
 *                                 +-\/-+           nRF24L01   CE, pin3 ------|
...
------x----------x--|<|-- 5V
 *                   NC      PB3  2|    |7  PB2 --- nRF24L01  SCK, pin5
--|<|---x-[22k]--|  LED
 ...
 *
 * Generic Linux devices are supported via SPIDEV, MRAA, RPi native via BCM2835,
or using LittleWire.
 *
 *  @note The SPIDEV option should work with most Linux systems supporting spi
userspace device. <br>
 *
 * <br>
 ...
 *
 *
 * 1. Install prerequisites if there are any (MRAA, LittleWire libraries, setup
SPI device etc)
...
SPI device etc)
 * @note See the <a
href="http://iotdk.intel.com/docs/master/mraa/index.html">MRAA </a>
documentation for more info on installing MRAA <br>
 * 2. Make a directory to contain the RF24 and possibly RF24Network lib and
enter it
 * @code
 *  mkdir ~/rf24libs
 *  cd ~/rf24libs
 ...
 * 5. Configure build environment using @code ./configure @endcode script. It
auto detectes device and build environment. For overriding autodetections, use
command-line switches, see @code ./configure --help @endcode for description.
 ...
 * 3. Follow <a href="Linux.html">Linux installation steps</a> to install RF24
libraries
 *
 *
 * <br><br><br>
 ...
 *
 * RF24 supports a variety of Linux based devices via various drivers. Some
boards like RPi can utilize multiple methods
 ...
 * MRAA is not included. See the <a href="MRAA.html">MRAA</a> platform page for
more information.
 *
 ...
 * Build using **SPIDEV**
 *
 * 1. Make sure that spi device support is enabled and /dev/spidev\<a\>.\<b\> is
present
 ...
 * In general, use @code RF24 radio(<ce_pin>, <a>*10+<b>); @endcode for proper
SPIDEV constructor to address correct spi device at /dev/spidev\<a\>.\<b\>
 *
...
https://github.com/jscrane/RF24-rpi  <br>
 ...
 * @note For python3 in Raspbian, it's needed to manually link python boost
library, like this:
 * @code sudo ln -s /usr/lib/arm-linux-gnueabihf/libboost_python-py34.so
/usr/lib/arm-linux-gnueabihf/libboost_python3.so @endcode
 *
 ...
 * @note Build takes several minutes on arm-based machines. Machines with RAM
<1GB may need to increase amount of swap for build.
 *
 * 4. Install the library
 * @code sudo ./setup.py install  @endcode
 * See the additional <a href="pages.html">Platform Support</a> pages for
information on connecting your hardware  <br>
 * See the included <a href="pingpair_dyn_8py-example.html">example </a> for
usage information.
 *
 * 5. Running the Example
 * Edit the pingpair_dyn.py example to configure the appropriate pins per the
above documentation:
 * @code nano pingpair_dyn.py   @endcode
 * Configure another device, Arduino or RPi with the <a
href="pingpair_dyn_8py-example.html">pingpair_dyn</a> example <br>
 * Run the example
 ...
 *  - ssh passwordless access to target machine
(https://linuxconfig.org/passwordless-ssh)
 *  - sudo of a remote user without password
 ...
 *  @code export
PATH=$PATH:/your/dir/rpi-tools/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian-x64/bin
@endcode
 *
 ...
 * 3. build
 * @code make @endcode
 * 4. (opt) install library to cross-compilation machine into cross-exvironment
- important for compilation of examples
 * @code sudo make install @endcode
 ...
 *
 * Prerequisites:
 * - Python setuptools must be installed on both target and cross-compilation
machines
 ...
 *
 * Installation steps:
 * 1. Assure having libboost-python-dev library in your cross-compilation
environment. Alternatively, you can install it into your target machine and copy
/usr and /lib directories to the cross-compilation machine.
 * For example
 * @code
 * mkdir -p rpi_root && rsync -a pi@target_linux_host:/usr :/lib rpi_root
 * export CFLAGS="--sysroot=/your/dir/rpi_root
-I/your/dir/rpi_root/usr/include/python2.7/"
 * @endcode
 *
 ...
 *
 * The RF24 driver can be build as a static library with Atmel Studio 7 in order
to be included as any other library in another program for the XMEGA family.
 *
 ...
 * As not all files are required, copy the following directory structure in the
project:
 *
 * @code
 * utility\
 ...
 * In the file where the **main()** is put the following in order to update the
millisecond functionality:
 *
 * @code
 * ISR(TCE0_OVF_vect)
 * {
 ...
 * @endcode
 *
 * Declare the rf24 radio with **RF24 radio(XMEGA_PORTC_PIN3,
XMEGA_SPI_PORT_C);**
 *
 * First parameter is the CE pin which can be any available pin on the uC.
 *
 * Second parameter is the CS which can be on port C (**XMEGA_SPI_PORT_C**) or
on port D (**XMEGA_SPI_PORT_D**).
 *
 * Call the **__start_timer()** to start the millisecond timer.
 *
 * @note Note about the millisecond functionality:<br>
 *
 * 	The millisecond functionality is based on the TCE0 so don't use these
pins as IO.<br>
 * 	The operating frequency of the uC is 32MHz. If you have other frequency
change the TCE0 registers appropriatly in function **__start_timer()** in
**compatibility.c** file for your frequency.
 *
 * @page Portability RF24 Portability
 *
 * The RF24 radio driver mainly utilizes the <a
href="http://arduino.cc/en/reference/homePage">Arduino API</a> for GPIO, SPI,
and timing functions, which are easily replicated
 ...
 * **RF24/utility**
 *
 * The RF24 library now includes a basic hardware template to assist in porting
to various platforms. <br> The following files can be included
 * to replicate standard Arduino functions as needed, allowing devices from
ATTiny to Raspberry Pi to utilize the same core RF24 driver.
 *
 * | File               |                   Purpose |
 *
|--------------------|------------------------------------------------------------------------------|
 * | RF24_arch_config.h | Basic Arduino/AVR compatibility, includes for
remaining support files, etc   |
...
|
 * | compatibility.h    | Provides standardized timing (millis(), delay())
methods                     |
 ...
 *
 * 1. The main detection for Linux devices is done in the configure script, with
the includes.h from the proper hardware directory copied to
RF24/utility/includes.h <br>
 * 2. Secondary detection is completed in RF24_config.h, causing the include.h
file to be included for all supported Linux devices <br>
 * 3. RF24.h contains the declaration for SPI and GPIO objects 'spi' and 'gpio'
to be used for porting-in related functions.
 *
 * <br>
 * @section Ported_Code Code
 * To have your ported code included in this library, or for assistance in
porting, create a pull request or open an issue at
